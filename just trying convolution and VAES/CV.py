...
### Linear section
        ...
        self.encoded_space_2log_var=nn.Linear(128,encoded_space_dim)
        
    def forward(self, x):
        x = self.encoder_cnn(x)
        x = self.flatten(x)
        x = self.encoder_lin(x)
        mu,log_var=self.encoded_space_2mu(x),self.encoded_space_2log_var(x)
        return mu,log_var
...
def train_epoch(encoder, decoder, device, dataloader, loss_fn, optimizer):
    ...
    for image_batch, _ in dataloader: # with "_" we just ignore the labels (the second element of the dataloader tuple)
        ...
        # print(mu,"\n",sigma)
        # Decode data
        encoded_data=mu+torch.exp(log_var*0.5)*torch.randn_like(log_var)#this line ensure normal distribution
        decoded_data = decoder(encoded_data)
        # Evaluate loss
        kl_div_loss=-0.5*torch.sum(1-torch.exp(log_var)+log_var-torch.pow(mu,2))#beta=0.11
